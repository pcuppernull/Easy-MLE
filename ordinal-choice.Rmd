...
Ordinal models take a dependent variable which is ordered. For instance, is a voter liberal, moderate, or conservative? Is a country's internet Not Free, Partially Free, or Free? The assumption with these models is that the potential outcomes of the dependent variable can theoretically be represented on a single continuum. If you believe that a liberal voter must become moderate before they become conservative, ordinal models are a justifiable choice -- but, if you think a voter could jump from liberal to conservative without first becoming moderate, and thus their ideology is not so much a single continuum, it would be advisable to use a multinomial model (the next `rmd`). 

Not sure if your dependent variable is ordinal or multinomial? Long (1995) recommends using a multinomial model. If you use a multinomial model on ordinal data, you'll lose efficiency (larger variance in your estimates), but your results will remain unbiased. But, if you use an ordinal model with a dependent variable that is truly multinomial, your estimates will be biased.
...
To estimate the model, we use the `polr` command from the `MASS` library. Specify `method = "probit"` to indicate that you wish to use an ordered probit model. Ordered logit models are also possible, but _____.

```{r}
model <- polr(as.ordered(y) ~ x1 + x2 + x3, method = "probit", data = data, Hess = T) # where y is the dependent variable; x1, x2, x3 are the independent variables (add as many as you want)
...
```{r}
x.mean <- cbind(mean(data$x1, na.rm = TRUE), 
             mean(data$x2, na.rm = TRUE),
             mean(data$x3, na.rm = TRUE))
x.max <- cbind(max(data$x1, na.rm = TRUE), 
             mean(data$x2, na.rm = TRUE),
             mean(data$x3, na.rm = TRUE))
x.min <- cbind(min(data$x1, na.rm = TRUE), 
             mean(data$x2, na.rm = TRUE),
             mean(data$x3, na.rm = TRUE))
```
...
This function takes six arguments: the model we estimated, the three vectors of `x` values, a string of the category names (for example: liberal, moderate, or conservative), and a second strings naming each `x` vector.

*Note: This function estimates the predicted probability that each `x` vector falls into each outcome category. As such, if your dependent variable `y` has four possible outcomes, you must add an extra section for "Outcome Category 4". Currently, this function is written for 1) a `y` variable with three possible outcomes, and 2) 3 `x` vectors on which to evalute first differences.
...
x.names <- c("Min GDP", "Mean GDP", "Max GDP")

oprob_fd(model, x.mean, x.max, x.min, categories, x.names)
```

That's it! The table presents the maximum likelihood estimates for across outcome category for each vector of `x` values you created. Again, this function is written foor a `y` dependent variable with three possible outcomes and three separate `x` vectors. Be sure to modify this accordingly for your own application.
