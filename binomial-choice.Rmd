...
## Application
Below, we have some survey data on affect towards Joe Biden. In addition to affect, we have each respondent's sex, age, education level, and binary indicators that indicate part afffiliation as democrat or republican.

Let's begin by loading in the data. We'll then create a binary `love_biden` variable, which take a value of 1 if affect towards biden is 80 or higher, and 0 otherwise.
...
#### Model
Next, we create out model, just like we wealked through above.
```{r}
...
```
Wow! Now that is a big effect. The "average" respondent in the data set who is a democrat has nearly 50% chance of "loving Biden". A Republican who otherwise has the same characteristics has only a tiny probability of "loving Biden". This is a more intuitive result.
