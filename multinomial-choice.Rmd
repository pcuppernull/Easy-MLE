...
Next, we turn to multinomial choice models. Much like binomial and ordinal models, multinomial models are defined by the nature of the dependent variable. In this case, the dependent variable is a choice between distinct outcomes that are *not* ordered in nature. For example, xxxx

There is some useful intuition behind these models to understand how they work. Multinomial models assume xyz
...
To estimate the model, we use the `multinom` command from the `nnet` library. This function will specify a multinomial logit model.
...
This function only takes two arguments: the model and one of the vectors. To compare first differences, you would then want to run this function for each vector. This function **MUST BE MODIFIED** depending on the number of possible outcomes in your dependent variable. I present two versions of the function: the first for 3 choices, the second for 4. Depending on the number of choices, modify the following components of the function:
  ...
  3. Create `choice` options - for three choices, we have `choice1`, `choice2`, and the `baseline` (which is choice 3). Each `choice` consists of each `exp()` term from the `denom` object, divided by the `denom` object. The `baseline` object always goes last and is calculated as 1 minus each `choice` object.
...
ci <- rbind(quantile(baseline, probs = c(.025, .5, .975)),
     ...
     # quantile(choice3, probs = c(.025, .5, .975)))
