...
As the name suggests, we use count models when our dependent variable is a *count* of something. In effect, our dependent variable takes values of non-negative integers (0, 1, 2, 3...). You can imagine plenty of research scenarios where this is the case: the number of battle deaths in a conflict, the number of x, y.

There are two main distributions for count models you'll tend to come across: poisson and negative binomial. Poisson models assume that the mean and variance of the distribution of the dependent variable are equal to each other. In most cases, this asssumption is not warranted and it is appropriate to use a negative binomial model. The code below assumes a negative binomial distribution -- if you want to check out tests for determining if the assumption of a poisson distribution is warranted, check out "dispersion tests".
...
Create your model here. `y` is your dependent variable, which MUST be a count variable (i.e. remember, non-negative integers only!). 

To estimate the model, we use the `glm.nb` command from the `MASS` library. Specify to indicate that you wish to use an ordered probit model. Ordered logit models are also possible, but _____.

```{r}
model <- glm.nb(y ~ x1 + x2 + x3, data = data)
 # where y is the dependent variable; x1, x2, x3 are the independent variables (add as many as you want)
# data is your data frame.
```

### Set Simulation Values

If you've gone through the code for other model types, the structure of our first differences calculations will be familiar to you. We begin by creating a few vectors of `X` values with which to run the maximum likelihood simulations. In effect, we plug in interesting values for each for the independent variables to observe the "most likely" dependent variables, per the specification of the model. One common way to do this is by varying one independent variable across a few substantively interesting values while holding all the other variable constant at their means in the data set. Below, I set three sets of `x` values.
